 ...
 */
@Slf4j @TestPropertySource(properties = { "batch.record-purge.enabled=true", "batch.record-purge.cron=0 45 6 * * ?", "batch.record-purge.days-to-keep=30" })
class RetrievalRecordPurgeJobIT extends AbstractBatchJobIntegrationTest {

        @Autowired
        private JobOperator jobOperator;

        @Autowired @Qualifier("retrievalRecordPurgeJob")
        ...
        private JobExecution runJob() throws Exception {
                ...
                return jobOperator.start(retrievalRecordPurgeJob, params);
        }

        /**
         * Test: RetrievalRecordPurgeJob purges old records while keeping recent ones.
         *
         * Given: 15 retrieval records exist (5 recent, 10 old) When: RetrievalRecordPurgeJob is executed with 30-day retention Then: Old records are purged, recent records remain
         */
        @Test
        void retrievalRecordPurgeJobPurgesOldRecords() throws Exception {
                ...
                List<ComicRetrievalRecord> allRecordsAfter = retrievalStatusService.getRetrievalRecords(null, null, null, null, Integer.MAX_VALUE);
                ...
                boolean hasOldRecords = allRecordsAfter.stream().anyMatch(r -> r.getComicName().startsWith("OldComic"));
         ...
         *
         * Given: No retrieval records exist When: RetrievalRecordPurgeJob is executed Then: Job completes successfully with no errors
         */
        @Test
        void retrievalRecordPurgeJobWithNoRecords() throws Exception {
                ...
                // Clear all records
                List<ComicRetrievalRecord> allRecords = retrievalStatusService.getRetrievalRecords(null, null, null, null, Integer.MAX_VALUE);
         ...
         *
         * Given: Job has already run and purged old records When: Job is run again Then: No errors occur, same records remain
         */
        @Test
        void retrievalRecordPurgeJobIsIdempotent() throws Exception {
                ...
                List<ComicRetrievalRecord> recordsAfterFirst = retrievalStatusService.getRetrievalRecords(null, null, null, null, Integer.MAX_VALUE);
                ...
                List<ComicRetrievalRecord> recordsAfterSecond = retrievalStatusService.getRetrievalRecords(null, null, null, null, Integer.MAX_VALUE);
