...
/**
 * Centralized scheduling triggers for all batch jobs.
 * Uses @Scheduled to call the appropriate job scheduler at the configured
 * times.
 *
 * <p>
...
public class SchedulerTriggers {
    ...
    @Autowired(required = false)
    private PeriodicJobScheduler metricsUpdateJobScheduler;
    ...
    public void triggerMetricsUpdate() {
        if (metricsUpdateJobScheduler != null) {
            metricsUpdateJobScheduler.executeScheduled();
        }
    }
}
