...
/**
 * Configuration for metrics system. Conditionally creates metrics beans based
 * on comics.metrics.enabled property.
 *
 * When metrics are enabled (default): - Creates collectors, repositories, and
 * services - Enables scheduled metrics updates and archiving
 *
 * When metrics are disabled: - Creates NoOpMetricsService that returns empty
 * data - Skips all collector and repository beans
 */
@Slf4j
@ToString
@Configuration
public class MetricsConfiguration {
    ...
    /**
     * Creates MetricsService when metrics are enabled. This is the main facade for
     * all metrics operations.
     */
    @Bean
    @ConditionalOnProperty(prefix = "comics.metrics", name = "enabled", havingValue = "true", matchIfMissing = true)
    public MetricsService metricsService(StorageMetricsCollector storageMetricsCollector,
            AccessMetricsCollector accessMetricsCollector, AccessMetricsRepository accessMetricsRepository,
        ...
        return new JsonMetricsService(storageMetricsCollector, accessMetricsCollector, accessMetricsRepository,
                metricsArchiver, metricsUpdateService);
    }

    /**
     * Creates NoOpMetricsService when metrics are disabled. Returns empty data for
     * all operations.
     */
    @Bean
    ...
    /**
     * Creates StorageMetricsCollector when metrics are enabled. Scans filesystem to
    ...
    /**
     * Creates AccessMetricsCollector when metrics are enabled. Tracks comic access
     * patterns and cache hit/miss rates. Uses event-driven persistence based on
     * persist-threshold.
     */
    @Bean
    @ConditionalOnProperty(prefix = "comics.metrics", name = "enabled", havingValue = "true", matchIfMissing = true)
    ...
    /**
     * Creates AccessMetricsRepository when metrics are enabled. Persists access
     * metrics to JSON files.
     */
    @Bean
    @ConditionalOnProperty(prefix = "comics.metrics", name = "enabled", havingValue = "true", matchIfMissing = true)
    public AccessMetricsRepository accessMetricsRepository(@Qualifier("gsonWithLocalDate") Gson gson,
    ...
    /**
     * Creates MetricsArchiver when metrics are enabled. Archives daily metrics
